...
parser.add_argument("--multi_identity", action="store_true", help="Path for the dataset.")
...
if opt.mode == 'train':
    ...
    generator.eval()
    dirs = []
    if opt.multi_identity:
        dirs = [item for item in current_session_dir.iterdir() if item.is_dir()]
    ...
    while images_left_to_generate > 0:
        ...
        number_of_images_to_generate = opt.chunk_size if images_left_to_generate > opt.chunk_size else images_left_to_generate
    #z = torch.rand(opt.number_of_generated_images, opt.latent_dim, device=device)
        z = Variable(Tensor(np.random.normal(0, 1, (number_of_images_to_generate, opt.latent_dim))))
        gen_imgs = generator(z)
        for i, img in enumerate(gen_imgs):
            if not opt.multi_identity:
                image_path = current_session_dir / f"generated_img_{images_left_to_generate - i}.png"
            else:
                identity = ""
                while True:
                ...
                image_path = image_path / f"generated_img_{i}.png"
